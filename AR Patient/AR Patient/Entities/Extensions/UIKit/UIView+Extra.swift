...
extension UISegmentedControl {
    ...
    func ensureiOS12Style() {
        if #available(iOS 13, *) {
            let tintColorImage = UIImage(color: Theme.color.btn2BG, size: CGSize(width: 1, height: 1))
            // Must set the background image for normal to something (even clear) else the rest won't work
            setBackgroundImage(UIImage(color: backgroundColor ?? .clear, size: CGSize(width: 1, height: 1)), for: .normal, barMetrics: .default)
            setBackgroundImage(tintColorImage, for: .selected, barMetrics: .default)
            setBackgroundImage(UIImage(color: tintColor.withAlphaComponent(0.2), size: CGSize(width: 1, height: 1)), for: .highlighted, barMetrics: .default)
            ...
            setTitleTextAttributes([NSAttributedString.Key.foregroundColor: Theme.color.btn2BG], for: .normal)
        }
    }
}
extension UIButton {
    func moveImageLeftTextCenter(imagePadding: CGFloat = 15.0){
        ...
        imageEdgeInsets = UIEdgeInsets(top: 10.0, left: imagePadding - imageViewWidth / 2, bottom: 10.0, right: 0.0)
        if(self.title(for: .normal)! == "KeybtnCallAgent".localize) {
            titleEdgeInsets = UIEdgeInsets(top: 0.0, left: (bounds.width - titleLabelWidth) / 2 - imageViewWidth - 80, bottom: 0.0, right: 0.0)
        } else if self.title(for: .normal)! == "VIEW" || self.title(for: .normal)! == "UNBLOCK" {
            titleEdgeInsets = UIEdgeInsets(top: 0.0, left: (bounds.width - titleLabelWidth) / 2 - imageViewWidth - 110, bottom: 0.0, right: 0.0)
        } else if self.title(for: .normal)! == "KeyCbtnDecline".localize {
            imageEdgeInsets = UIEdgeInsets(top: 10.0, left: (imagePadding - imageViewWidth / 2)-7, bottom: 10.0, right: 0.0)
            titleEdgeInsets = UIEdgeInsets(top: 0.0, left: (bounds.width - titleLabelWidth) / 2 - imageViewWidth - 30, bottom: 0.0, right: 0.0)
        } else {
            titleEdgeInsets = UIEdgeInsets(top: 0.0, left: (bounds.width - titleLabelWidth) / 2 - imageViewWidth - 30, bottom: 0.0, right: 0.0)
        }
    }
