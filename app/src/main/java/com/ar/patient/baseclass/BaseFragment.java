...
public class BaseFragment extends Fragment {
     ...
     * getActivity() callback will be called ONLY ONCE before the whole
    ...
    public boolean isOnline(boolean ShowProgress) {
        if (Utils.isOnline(getActivity())) {
            ...
            if (ShowProgress)
                showValidationDialog(getActivity(), getActivity().getString(R.string.msg_no_internet), null);
            return false;
        }
    }
    ...
    public void showProgress() {
        if (myProgressBar == null) {
            myProgressBar = MyProgressBar.create(getActivity());
    ...
    public void showSingleButtonDialog(final Context context, String msg, final DialogClickListener dialogClickListener) {
    ...
    public void showSingleButtonDialogNoRecord(AppCompatActivity activity, String msg, final DialogClickListener dialogClickListener) {
        new FancyGifDialog.Builder(activity)
                .setTitle(getString(R.string.app_name))
                .setMessage(msg)
                .setPositiveBtnBackground("#f78022")
                .setPositiveBtnText("Ok")
                .isCancellable(true)
                .OnPositiveClicked(new FancyGifDialogListener() {
                    @Override
    ...
    public void showValidationDialog(final Context context, String msg, final DialogClickListener dialogClickListener) {
        new FancyGifDialog.Builder(getActivity())
                .setTitle(getString(R.string.app_name))
                .setMessage(msg)
                .setPositiveBtnBackground("#000000")
    ...
    public void showDialog(final Context context, String msg, String oktext, String notext,
                           final DialogClickListener dialogClickListener) {
        new FancyGifDialog.Builder(getActivity())
                .setTitle(getString(R.string.app_name))
                .setMessage(msg)
                .setPositiveBtnBackground("#0375F3")
                .setPositiveBtnText(oktext)
                .setNegativeBtnBackground("#000000")
                .setNegativeBtnText(notext)
                .isCancellable(true)
                .OnPositiveClicked(new FancyGifDialogListener() {
                    @Override
                    public void OnClick() {
    ...
    public boolean checkCalenderPermission() {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
            if (ContextCompat.checkSelfPermission(getActivity(), Manifest.permission.WRITE_CALENDAR) != PackageManager.PERMISSION_GRANTED ||
                ...
                ActivityCompat.requestPermissions(getActivity(), new String[]{Manifest.permission.WRITE_CALENDAR, Manifest.permission.READ_CALENDAR}, Config.CODE_REQUEST_PERMISSION_CALENDER);
                return false;
            } else {
                return true;
            }
    ...
    public boolean checkCameraStoragePermission() {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
            if (ContextCompat.checkSelfPermission(getActivity(), Manifest.permission.WRITE_EXTERNAL_STORAGE) != PackageManager.PERMISSION_GRANTED ||
                ...
                ActivityCompat.requestPermissions(getActivity(), new String[]{Manifest.permission.WRITE_EXTERNAL_STORAGE, Manifest.permission.READ_EXTERNAL_STORAGE, Manifest.permission.CAMERA}, Config.CODE_REQUEST_PERMISSION_CAMERA);
                return false;
            } else {
                return true;
            }
    ...
    public boolean checkAgoraPermission() {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
            if (ContextCompat.checkSelfPermission(getActivity(), Manifest.permission.CAMERA) != PackageManager.PERMISSION_GRANTED || ContextCompat.checkSelfPermission(getActivity(), Manifest.permission.RECORD_AUDIO) != PackageManager.PERMISSION_GRANTED) {
                ActivityCompat.requestPermissions(getActivity(), new String[]{Manifest.permission.CAMERA, Manifest.permission.RECORD_AUDIO}, Config.CODE_REQUEST_PERMISSION_CAMERA);
                return false;
            } else {
                return true;
            }
