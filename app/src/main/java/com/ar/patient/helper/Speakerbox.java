...
public class Speakerbox implements TextToSpeech.OnInitListener {
    ...
    private final String voiceConfig;
    ...
    public Speakerbox(final Application application,String voiceConfig) {
    ...
    public void onInit(int status) {
        if (status == TextToSpeech.SUCCESS) {
            ...
            for (Voice tmpVoice : textToSpeech.getVoices()) {
                if (tmpVoice.getName().equalsIgnoreCase(voiceConfig)) {
                    v = tmpVoice;
                    break;
                }
            }
